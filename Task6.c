...
void *calculateCyclicColSum(void *object)
{
    ...
    for (int i = 0; i < sizeof(structure->buffer_array)/ sizeof(structure->buffer_array[0]); i++)
    {
    ...
    *(structure->thread_sum) += temp_sum;
    printf("\n \033[1;34m [-] \033[0m Printing Sum of Cols:  \033[33m%ld\033[0m\n\n", temp_sum); // Printing Local col Sum
    return NULL;
}

int **Get_Diagonaled_Array(){
    int **diagonal = (int **)malloc(((MATRIX_SIZE * 2) - 1) * sizeof(int *));
    for(int i = 0 ; i < (MATRIX_SIZE * 2) - 1; i++){
        diagonal[i] = (int *)malloc((MATRIX_SIZE * 2) * sizeof(int));
        for(int j = 0; j < MATRIX_SIZE * 2; j++) {
            diagonal[i][j] = 0; // Initialize all elements to 0
        }
    }
    for(int k = 0; k < (MATRIX_SIZE * 2) - 1; k ++){
        int row = 0;
        for(int i = 0 ; i < MATRIX_SIZE; i++){
            for(int j = 0 ; j < MATRIX_SIZE; j++){
                if(i+j == k){
                    diagonal[k][row++] = matrix[i][j];
                }
            }
        }
    }
...
int main()
{
    ...
    int **diagonal_array = Get_Diagonaled_Array();

    for (int i = 0; i < NUM_THREADS; i++) {
        thread_args[i].thread_sum = (long *)malloc(sizeof(long));
        *(thread_args[i].thread_sum) = 0;
    }
    // Loop for Cyclic distribution of cols among 4 Threads 
    for (int i = 0; i < MATRIX_SIZE; i++)
    {
        if(num == NUM_THREADS){ // setting thread number back to 0 if thread exceeds 4 due to 1024 loop size
            num =0;
        }
        ...
        thread_args[num].buffer_array = diagonal_array[i];
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        distsum += *(thread_args[i].thread_sum); // Accumulate the added col sums
    }
    for (int i = 0; i < NUM_THREADS; i++) {
        free(thread_args[i].thread_sum);
    }
